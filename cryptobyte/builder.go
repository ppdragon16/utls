...
type Builder struct {
	...
	pendingIsASN1  bool
	inContinuation *bool
	scratch        [4]byte // zero-alloc scratch for length prefixes
}
...
func (b *Builder) addLengthPrefixed(lenLen int, isASN1 bool, f BuilderContinuation) {
	...
	offset := len(b.result)
	if len(b.result)+lenLen < lenLen {
		b.err = errors.New("cryptobyte: length overflow")
		return
	}
	b.result = append(b.result, b.scratch[:lenLen]...)

	if b.inContinuation == nil {
		b.inContinuation = new(bool)
	}

	child := builderPool.Get().(*Builder)
